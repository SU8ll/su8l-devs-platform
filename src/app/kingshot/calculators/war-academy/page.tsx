...
export default function WarAcademyCalculator() {
  ...
  const levels = Array.from({ length: 35 }, (_, i) => ({ value: i + 1, label: t("calculator.levelOption", { level: String(i + 1) }) }))
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-16">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        <h1 className="text-4xl font-bold mb-2"><span className="gradient-text">{t("calculator.warAcademy")}</span></h1>
        <p className="text-[#a0a0b0] mb-8">{t("calculator.calculateWarAcademy")}</p>
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-white/10">
                        ...
                        <th className="text-right py-3 px-4 text-[#7c3aed]">{t("calculator.academyBooks")}</th>
