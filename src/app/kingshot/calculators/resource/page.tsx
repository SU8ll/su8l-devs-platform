  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-16">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        <h1 className="text-4xl font-bold mb-2"><span className="gradient-text">Resource Calculator</span></h1>
        <p className="text-[#a0a0b0] mb-8">Calculate gathering and production times for resources</p>

        <GlassCard className="mb-8">
          <GlassCardContent className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                <label className="block text-sm text-[#a0a0b0] mb-1">Resource Type</label>
                <Select options={resourceTypes} value={resource} onChange={(e) => setResource(e.target.value)} />
              </div>
              <div>
                <label className="block text-sm text-[#a0a0b0] mb-1">Amount Needed</label>
                <input type="number" value={amount} onChange={(e) => setAmount(Number(e.target.value))} className="input-glass w-full" min={1} />
              </div>
              <div>
                <label className="block text-sm text-[#a0a0b0] mb-1">Gathering Bonus %</label>
                <input type="number" value={bonus} onChange={(e) => setBonus(Number(e.target.value))} className="input-glass w-full" min={0} max={200} />
              </div>
            </div>
            <Button onClick={handleCalculate} className="w-full">Calculate Time</Button>
          </GlassCardContent>
        </GlassCard>
